...
describe BitmapEditor do
  ...
  it 'indicates if command letter not valid' do
    ...
    expect { subject.run("show.txt") }.to output("unrecognised command :(\n").to_stdout
  end
  ...
  it 'shows the bitmap with command S' do
    ...
    content = ["I 3 2", "S"]
    allow(File).to receive(:open).with(filename).and_return(content)
    expect { subject.run("show.txt") }.to output("OOO\nOOO\n").to_stdout
  end
  ...
  it 'raises and error if there is no bitmap' do
    ...
    content = ["H 1 2 2 B"]
    allow(File).to receive(:open).with(filename).and_return(content)
    expect { subject.run("show.txt") }.to raise_error 'You need to create a bitmap first'
  end
